...
{
	public class FormMemobusTcpServer : HslFormContent
	{
		private MemobusTcpServer tcpServer;
		...
		{
			MemobusTcpServer memobusTcpServer = tcpServer;
			if (memobusTcpServer != null)
			{
				memobusTcpServer.ServerClose();
			}
		}
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			else
			{
				try
				{
					tcpServer = new MemobusTcpServer();
					tcpServer.ActiveTimeSpan = TimeSpan.FromHours(1.0);
					tcpServer.OnDataReceived += BusTcpServer_OnDataReceived;
					tcpServer.ServerStart(result);
		...
		private void InitializeComponent()
		{
			...
			label11.TabIndex = 29;
			label11.Text = "本服务器不是严格的Memobus协议，仅支持和HSL组件完美通信。";
			button11.Enabled = false;
			...
			userControlHead1.ProtocolInfo = "扩展 Memobus";
			...
			base.Name = "FormMemobusTcpServer";
			base.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
			Text = "MemobusTcp虚拟服务器【数据支持线圈，输入线圈，寄存器，输入寄存器，扩展寄存器，扩展输入寄存器】";
