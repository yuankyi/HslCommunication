...
{
	public class FormVigorSerial : HslFormContent
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			{
				VigorSerial vigorSerial = vigor;
				if (vigorSerial != null)
				{
					vigorSerial.Close();
				}
				vigor = new VigorSerial();
				vigor.LogNet = base.LogNet;
				try
				{
					vigor.SerialPortInni(delegate(SerialPort sp)
					{
					...
					panel2.Enabled = true;
					userControlReadWriteOp1.SetReadWriteNet(vigor, "D100");
				}
				catch (Exception ex)
				{
		...
		{
			OperateResult<byte[]> operateResult = vigor.ReadFromCoreServer(SoftBasic.HexStringToBytes(textBox13.Text));
		...
		private void test1()
		{
			OperateResult<bool[]> operateResult = vigor.ReadBool("M100", 10);
			if (operateResult.IsSuccess)
			{
		...
		private void test3()
		{
			short content = vigor.ReadInt16("D100").Content;
			ushort content2 = vigor.ReadUInt16("D100").Content;
			int content3 = vigor.ReadInt32("D100").Content;
			uint content4 = vigor.ReadUInt32("D100").Content;
			long content5 = vigor.ReadInt64("D100").Content;
			ulong content6 = vigor.ReadUInt64("D100").Content;
			float content7 = vigor.ReadFloat("D100").Content;
			double content8 = vigor.ReadDouble("D100").Content;
			string content9 = vigor.ReadString("D100", 10).Content;
		}
		...
		private void test4()
		{
			...
			vigor.Write("D100", 5u);
			vigor.Write("D100", 5L);
			vigor.Write("D100", 5uL);
			vigor.Write("D100", 5f);
			vigor.Write("D100", 5.0);
			vigor.Write("D100", "12345678");
		}

		private void test5()
		{
			OperateResult<byte[]> operateResult = vigor.Read("D100", 10);
			if (operateResult.IsSuccess)
			{
				int num = vigor.ByteTransform.TransInt32(operateResult.Content, 0);
				float num2 = vigor.ByteTransform.TransSingle(operateResult.Content, 4);
				short num3 = vigor.ByteTransform.TransInt16(operateResult.Content, 8);
		...
		private void test6()
		{
			OperateResult<UserType> operateResult = vigor.ReadCustomer<UserType>("D100");
			if (operateResult.IsSuccess)
			{
				UserType content = operateResult.Content;
			}
			vigor.WriteCustomer("D100", new UserType());
			vigor.LogNet = new LogNetSingle(Application.StartupPath + "\\Logs.txt");
		}
		...
		private void InitializeComponent()
		{
			...
			userControlHead1.ProtocolInfo = "Vigor vs protocol";
