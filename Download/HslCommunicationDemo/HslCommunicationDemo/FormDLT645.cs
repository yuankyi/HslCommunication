...
{
	public class FormDLT645 : HslFormContent
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				try
				{
					dLT645.SerialPortInni(delegate(SerialPort sp)
					{
					...
					panel2.Enabled = true;
					userControlReadWriteOp1.SetReadWriteNet(dLT645, "00-00-00-00");
				}
				catch (Exception ex)
				{
		...
		private void button25_Click(object sender, EventArgs e)
		{
			try
			{
				OperateResult<byte[]> operateResult = dLT645.Read(textBox6.Text, ushort.Parse(textBox9.Text));
				if (operateResult.IsSuccess)
				{
					for (int i = 0; i < operateResult.Content.Length; i++)
					{
						operateResult.Content[i] = (byte)(operateResult.Content[i] - 51);
					}
		...
		{
			OperateResult operateResult = dLT645.ActiveDeveice();
			if (operateResult.IsSuccess)
			{
		...
		{
			OperateResult<string> operateResult = dLT645.ReadAddress();
			if (operateResult.IsSuccess)
			{
		...
		{
			OperateResult operateResult = dLT645.BroadcastTime(DateTime.Now);
			if (operateResult.IsSuccess)
			{
				textBox12.Text = string.Format("[{0:HH:mm:ss}] BroadcastTime Success", DateTime.Now);
			}
			else
			{
				MessageBox.Show("Read failed: " + operateResult.Message);
		...
		{
			OperateResult operateResult = dLT645.WriteAddress(textBox1.Text);
			if (operateResult.IsSuccess)
			{
		...
		{
			OperateResult<string[]> operateResult = dLT645.ReadStringArray(textBox1.Text);
			if (operateResult.IsSuccess)
			{
		...
		private void InitializeComponent()
		{
			...
			checkBox5.Location = new System.Drawing.Point(631, 5);
			...
			button2.Enabled = false;
			button2.Location = new System.Drawing.Point(837, 27);
			...
			button1.Location = new System.Drawing.Point(740, 27);
			...
			panel2.MinimumSize = new System.Drawing.Size(800, 525);
			panel2.Name = "panel2";
			...
			userControlHead1.ProtocolInfo = "DLT 645 2007";
